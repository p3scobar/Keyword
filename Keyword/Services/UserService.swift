...
struct UserService {
    ...
    static func followUsers(userIds: [String]) {
        let urlString = "\(baseUrl)/followusers"
        let url = URL(string: urlString)!
        let token = Model.shared.token
        let header: HTTPHeaders = ["Authorization":"Bearer \(token)"]
        let params: Parameters = ["userIds":userIds]
        Alamofire.request(url, method: .post, parameters: params, encoding: URLEncoding.default, headers: header).responseJSON { (response) in
    ...
    static func login(email: String, password: String, completion: @escaping (Bool, String?) -> Void) {
        ...
        Alamofire.request(url, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            ...
            guard let json = response.result.value as? [String:Any],
                let resp = json["response"] as? [String:Any],
                let userData = resp["user"] as? [String:Any] else {
    ...
    static func suggestedUsers(completion: @escaping ([User]) -> Void) {
        DispatchQueue.global(qos: .background).async {
            let urlString = "\(baseUrl)/suggestedusers"
            let url = URL(string: urlString)!
            let token = Model.shared.token
            let headers: HTTPHeaders = ["Authorization": "Bearer \(token)"]
            ...
            Alamofire.request(url, method: .post, parameters: params, encoding: URLEncoding.default, headers: headers).responseJSON { (response) in
                ...
                guard let json = response.result.value as? [String:Any],
                    let resp = json["response"] as? [String:Any],
                    let people = resp["users"] as? [[String:Any]] else { return }
...
internal func setupDefaultaAvatar() {
    ...
    UserService.updateProfilePic(image: image, completion: { imageURL in
